    ...
    <div className="absolute inset-x-0 top-0 flex items-center justify-between px-3 pt-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center rounded-[var(--radius-md)] bg-bg-elevated shadow-elevation-2">
                        ...
                        <>
                          ...
                          {remainingCount > 0 && !spotlightedInOverflow && (
                            <Avatar key="overflow-count" className="size-5">
                              <AvatarFallback className="bg-paids-neutral-200 text-[length:var(--text-3xs)] font-semibold text-text-primary">+{remainingCount}</AvatarFallback>
                            </Avatar>
                          )}
                        </>
                      )
                    ...
                    <div key={user.id}>
                      ...
                      <div className="px-2 py-1">
                        <Button
                          ...
                          size="sm"
                          className="h-7 w-full rounded-[var(--radius-sm)] text-[length:var(--text-xs)]"
          ...
          <div className="flex items-center p-1">
            <button className="flex items-center rounded px-1.5 py-1 text-[length:var(--text-base)] font-medium text-text-primary transition-colors hover:bg-bg-elevated-hover">
              Share
            </button>
          </div>
          <div className="self-stretch w-px bg-border-secondary" />
          ...
          <div className="flex items-center p-1">
            <button className="flex items-center rounded px-2 py-1 text-[length:var(--text-base)] font-medium text-text-primary transition-colors hover:bg-bg-elevated-hover">
              Present
            </button>
          </div>
          <div className="self-stretch w-px bg-border-secondary" />
          ...
          <div className="flex items-center gap-1 p-1">
            <button className="flex h-7 items-center rounded-[var(--radius-sm)] border border-border-brand px-2.5 transition-colors hover:bg-bg-brand/5">
              <ShinyText
                text="Upgrade"
                speed={1}
                delay={2}
            ...
            <Button variant="primary" size="sm" className="h-7 gap-1.5 rounded px-2.5 text-[length:var(--text-base)] font-medium">
...
function SlideGapInsert({ onNewSlideWithPrompt, gapId, openGapId, onOpenGap }: { onNewSlideWithPrompt: () => void; gapId: number; openGapId: number | null; onOpenGap: (id: number | null) => void }) {
  const isMenuOpen = openGapId === gapId
  const [hovered, setHovered] = useState(false)
  const timeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null)

  // Reset hovered when another gap takes over
  useEffect(() => {
    if (openGapId !== null && openGapId !== gapId) {
      setHovered(false)
    }
  }, [openGapId, gapId])
  ...
  return (
    ...
    >
      {/* Invisible hover zone — full width, tall enough to catch cursor */}
      <div
      ...
      <div
        ...
        style={{ top: -10, bottom: -10, opacity: visible ? 1 : 0, transition: "opacity 0.1s" }}
      >
        <div className="absolute inset-x-0 top-1/2 h-px -translate-y-1/2 bg-border-brand" />
        <DropdownMenu open={isMenuOpen} onOpenChange={(open) => { onOpenGap(open ? gapId : null); if (!open) setHovered(false) }}>
          <DropdownMenuTrigger asChild>
            <button
              className="pointer-events-auto z-10 flex size-5 items-center justify-center rounded-full border border-border-brand bg-bg-elevated text-text-brand shadow-elevation-1 transition-transform hover:scale-110"
              onMouseEnter={show}
              onMouseLeave={hide}
            >
          ...
          <DropdownMenuContent side="right" sideOffset={12} align="center" className="w-[200px]">
...
function Filmstrip({ activeSlide, onSlideSelect, slides, onNewSlideWithPrompt, generatingSlideId }: { activeSlide: number; onSlideSelect: (id: number) => void; slides: typeof SLIDES; onNewSlideWithPrompt: () => void; generatingSlideId: number | null }) {
  ...
  const [openGapId, setOpenGapId] = useState<number | null>(null)
          ...
          {isCollapsed ? (
            ...
            >
              ...
              <div className="px-2 pb-2 pt-1.5">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="tertiary" size="xs" className="h-7 w-full text-[length:var(--text-base)] font-medium">
                      New slide
                    </Button>
                  </DropdownMenuTrigger>
                ...
                <AnimatePresence mode="wait" initial={false}>
                      ...
                      {slides.map((slide, idx) => {
                        ...
                        return (
                          <React.Fragment key={slide.id}>
                            {idx > 0 && <SlideGapInsert onNewSlideWithPrompt={onNewSlideWithPrompt} gapId={idx} openGapId={openGapId} onOpenGap={setOpenGapId} />}
                            <button
                  ...
                  {mode === "list" && (
                    ...
                    >
                      {slides.map((slide, idx) => (
                        <React.Fragment key={slide.id}>
                          {idx > 0 && <SlideGapInsert onNewSlideWithPrompt={onNewSlideWithPrompt} gapId={1000 + idx} openGapId={openGapId} onOpenGap={setOpenGapId} />}
                          <ListSlideRow
                            slide={slide}
                                  ...
                                  <div className="flex flex-col gap-1 pb-1">
                                    {sections.map((section) => (
                                      <button
                                        ...
                                        className={cn(
                                          "flex items-center gap-2 rounded px-2 py-1.5 text-left text-[length:var(--text-base)] transition-colors",
    ...
    <div className="absolute inset-x-0 bottom-3 flex items-center justify-center">
      ...
      <div className="flex items-center rounded-[var(--radius-md)] bg-bg-elevated shadow-elevation-3">
        ...
        <div className="flex items-center p-1">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="flex items-center gap-2 rounded pl-2 pr-3 py-1 text-[length:var(--text-base)] font-medium text-text-primary transition-colors hover:bg-bg-elevated-hover">
        ...
        <div className="flex items-center p-1">
          <button
            ...
            className="flex items-center gap-2 rounded pl-2 pr-3 py-1 text-[length:var(--text-base)] font-medium text-text-primary transition-colors hover:bg-bg-elevated-hover"
          >
    ...
    <div className="absolute bottom-3 right-3 flex w-9 flex-col items-center overflow-hidden rounded-[var(--radius-md)] bg-bg-elevated shadow-elevation-2">
      ...
      <button className="flex w-full items-center p-0.5 transition-colors hover:bg-bg-elevated-hover">
        <div className="flex w-full flex-col items-center justify-center gap-1.5 rounded py-1.5">
          ...
          <span className="text-[length:var(--text-2xs)] font-medium leading-none text-text-primary">100%</span>
        </div>
      </button>
    </div>
  )
...
export default function NewEditorPage() {
  ...
  const handleInsertSlide = () => {
    ...
    setTimeout(() => {
      ...
      audio.volume = 0.3
      audio.play().catch(() => {})
      // Show toast only if user is not currently viewing the generated slide
      if (activeSlideRef.current !== newId) {
